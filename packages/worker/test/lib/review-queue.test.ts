...
import { createReviewQueueMessage, parseReviewQueueMessage } from '../../src/lib/review-queue.js';
...
export async function runReviewQueueTests(): Promise<void> {
  ...
  assert.throws(
    () => parseReviewQueueMessage({ type: 'review_requested', reviewId: 'rev_abcd1234', queuedAt: new Date().toISOString(), openrouterApiKey: '' }),
    /openrouterApiKey/
  );
}
