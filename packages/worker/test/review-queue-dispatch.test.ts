...
import { createReviewQueueMessage } from '../src/lib/review-queue.js';
import { dispatchReviewToRunner, handleReviewQueueDispatch } from '../src/lib/review-dispatch.js';
...
export async function runReviewQueueDispatchTests(): Promise<void> {
  {
    ...
    const { message, getRetryCount } = createReviewMessage(
      createReviewQueueMessage('rev_abcd1234', 'ghp_user_token_123', 'provider_user_token_123', 'or_user_token_123')
    );
    const env = {
      ReviewRunner: {
    ...
    assert.equal(handoffBodies[0]?.providerApiKey, 'provider_user_token_123');
