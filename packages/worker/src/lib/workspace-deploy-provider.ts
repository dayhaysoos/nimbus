...
class CloudflareWorkersAssetsProvider implements WorkspaceDeployProvider {
  ...
  async createDeployment(input: WorkspaceDeployCreateInput): Promise<WorkspaceDeployCreateResult> {
    ...
    const uploadResponse = await this.request(`/accounts/${this.accountId}/workers/scripts/${this.projectName}/assets/upload`, {
      method: 'POST',
      body: JSON.stringify({
        deployment_id: input.deploymentId,
        output_dir: input.outputDir,
        artifact_sha256: input.outputBundle.sha256,
        artifact_bytes: input.outputBundle.bytes.byteLength,
        artifact_base64: toBase64(input.outputBundle.bytes),
      }),
    });
    const uploadParsed = await this.parseCloudflareResult(uploadResponse);
    const uploadResult =
      typeof uploadParsed.result === 'object' && uploadParsed.result !== null
        ? (uploadParsed.result as Record<string, unknown>)
        : {};
    const assetUploadId =
      typeof uploadResult.upload_id === 'string' && uploadResult.upload_id.trim()
        ? uploadResult.upload_id.trim()
        : `upload_${input.deploymentId}`;
