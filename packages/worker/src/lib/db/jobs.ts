import type { JobListItem, JobPhase, JobRecord, JobResponse, JobStatus } from '../../types.js';
...
/** Allocates a stable ID for a persisted checkpoint job. */
export function generateJobId(): string {
  return generatePrefixedId('job');
}

/** Persists a new checkpoint job in queued state. */
...
export async function getJob(db: D1Database, id: string): Promise<JobResponse | null> {
  const result = await db.prepare('SELECT * FROM jobs WHERE id = ?').bind(id).first<JobRecord>();
...
/** Atomically claims a queued checkpoint job for execution. */
...
/** Appends a sequenced checkpoint job event and returns its event sequence number. */
export async function appendJobEvent(
  ...
  await db
    ...
    .bind(input.jobId, input.attemptNo ?? 0, seq, input.eventType, input.phase, JSON.stringify(input.payload))
    .run();
...
/** Updates persisted job status and any supplied execution metadata columns. */
